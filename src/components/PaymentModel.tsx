...
import { CreditCard, Printer, Save, Loader2, X } from "lucide-react";
...
export default function PaymentModal({ onClose }: PaymentModalProps) {
  ...
  // Initialize unique receipt number exactly once on mount
  useEffect(() => {
    const now = new Date();
  ...
  const rawBalance = Math.max(0, numTotal - numPaid);

  // Auto-fill paid amount if payment type is "Full Payment"
  useEffect(() => {
    if (form.type === "Full Payment" && form.total) {
      setForm((f) => ({ ...f, paid: f.total }));
    }
  }, [form.type, form.total]);

  const handleSave = async () => {
    if (!form.name.trim() || !form.college.trim() || !form.billing || !form.paid || !form.courseName) {
    ...
    try {
      ...
      await fetch(webAppUrl, {
        ...
        body: JSON.stringify({
          ...
          paidAmount: numPaid,
          balanceAmount: rawBalance, // Sent silently to maintain sheet scheme architecture
    ...
    pWin.document.write(`
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; font-family: system-ui, -apple-system, sans-serif; }
          ...
          .g { display: grid; grid-template-columns: 1.1fr 1fr; gap: 20px; font-size: 11px; align-items: start; }
      ...
      <body>
        ...
        <div class="box">
          ...
          <div class="g">
            <div class="col">
              <div><div style="font-size:8.5px; text-transform:uppercase; color:#64748b; font-weight:700;">Student Participant</div><div style="font-size:12px; font-weight:600; color:#1e293b; margin-top:1px;">${form.name}</div></div>
    ...
    <div className="fixed inset-0 z-[999] bg-zinc-950/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-2xl rounded-3xl border border-zinc-100 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="px-8 py-4 border-t border-zinc-100 bg-zinc-50 flex gap-3 justify-end items-center rounded-b-3xl">
          <button 
            onClick={onClose} 
            className="px-5 py-2.5 rounded-xl text-xs font-semibold text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100 transition-colors"
          >
            Cancel
          </button>
          <button 
