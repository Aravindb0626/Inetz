...
import { Search, Filter, RefreshCw, UserPlus, Users, Wallet, AlertCircle, Calendar, RotateCcw } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-zinc-200/80 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black uppercase text-zinc-400 tracking-wider">Total Enrolled Students</p>
            <h3 className="text-2xl font-black text-zinc-900 mt-1">{summary.totalStudents.toLocaleString("en-IN")}</h3>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-emerald-100/80 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black uppercase text-emerald-600 tracking-wider">Total Fees Collected</p>
            <h3 className="text-2xl font-black text-emerald-700 mt-1">₹{summary.totalCollected.toLocaleString("en-IN")}</h3>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-amber-100/80 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black uppercase text-amber-600 tracking-wider">Outstanding Balance</p>
            <h3 className="text-2xl font-black text-amber-700 mt-1">₹{summary.totalPending.toLocaleString("en-IN")}</h3>
          </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-zinc-200/80 shadow-sm space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-100 pb-4">
          <div>
            <h2 className="text-base font-black text-zinc-900 uppercase tracking-tight">Student Directory</h2>
            <p className="text-xs text-zinc-400 font-medium mt-0.5">
              Domain: <span className="font-bold text-zinc-700">{domainFilter}</span>
              {(fromDate || toDate) && (
        ...
        {/* INPUTS ROW: SEARCH + DOMAIN + DATE RANGE */}
